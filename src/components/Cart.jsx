...
const Cart = ({ noimage }) => {
  ...
  useEffect(() => {
    ...
    if (cart) {
      cart.map((data) => {
        tot = tot + data.price * data.quantity;
        setTotal(tot);
      });
    }
  }, [cart]);
  ...
  return (
    <motion.div
      {...slideIn}
      className="fixed z-50 top-0 left-0 w-310 md:w-508 bg-lightOverlay backdrop-blur-md shadow-md h-screen"
    >
      <div className="w-full flex items-center justify-between py-4 pb-12 px-6">
        ...
        <p className="text-2xl text-headingColor font-semibold">سبد</p>
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6  gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            ...
            <div className="bg-zinc-800 rounded-t-[60px] w-full h-[35%] flex flex-col items-center justify-center px-4 py-6 gap-24">
              <div className="w-full flex items-center justify-evenly">
                <p className="text-3xl text-zinc-500 font-semibold">جمع کل</p>
                <p className="text-3xl text-blue-500 font-semibold flex items-center justify-center gap-1">
                  {total}
                </p>
              </div>
          ...
          <>
            <h1 className="text-3xl text-primary  md:pr-12 font-primary">
              سبد شما خالیست
            </h1>
          </>
        )}
...
export const CartItemCard = ({ index, data, noimage }) => {
  const cart = useSelector((state) => state.cart);
  const user = useSelector((state) => state.user);
  ...
  useEffect(() => {
    setItemTotal(data.price * data.quantity);
  }, [itemTotal, cart]);
